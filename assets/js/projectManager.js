class ProjectManager {
    ...
    createProjectCard(project) {
        return `
            <div x-show="activeFilter === 'all' || ${project.category.map(cat => `activeFilter === '${cat}'`).join(' || ')}"
                x-transition
                ...
                <div class="p-6">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        ${project.technologies.map(tech => `<span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-sm">${tech}</span>`).join('')}
                    </div>
                    <div class="flex justify-between items-center">
                        <a target="_blank" href="${project.links.demo}" class="text-${project.image.gradient.from} hover:text-${project.image.gradient.to} font-medium">View Project →</a>
